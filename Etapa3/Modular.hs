                    ...
                    localRec i =
                        if (i < (length xs)) then
                            let 
                                ...
                                elem = [f (head (snd splitted))]
                                secondHalf = (tail (snd splitted)) in
                            [firstHalf ++ elem ++ secondHalf] ++ (localRec (i + 1))
                        else
                            []   
                    in localRec 0
...
partitions xs = case xs of
                ...
                otherwise -> let
                        elem = (head xs)
                        list_elem = [elem]
                        old_miniparti = (partitions (tail xs))
                        new_miniparti = [p | mini_mini_parti <- old_miniparti, p <- mapSingle (\l -> elem:l) mini_mini_parti]
                        other = map (\_mini_parti -> list_elem:_mini_parti) old_miniparti
                        in  other ++ new_miniparti
...
isModule set graph = let 
                        inNeighSets = S.map (\node -> (inNeighbors node graph)) set
                        outNeighSets = S.map (\node -> (outNeighbors node graph)) set
                        filteredInSets = S.map (\mini_set -> (S.difference mini_set set)) inNeighSets
                        filteredOutSets = S.map (\mini_set -> (S.difference mini_set set)) outNeighSets
                        firstSetIn = S.elemAt 0 filteredInSets
                        firstSetOut = S.elemAt 0 filteredOutSets
                        checkIn = S.foldl (\acc mini_set -> if (firstSetIn /= mini_set) then False else acc) True filteredInSets
                        checkOut = S.foldl (\acc mini_set -> if (firstSetOut /= mini_set) then False else acc) True filteredOutSets
                    in (checkIn && checkOut)
...
isModularPartition :: Ord a
                   ...
                   -> Graph a
                   -> Bool
isModularPartition partition graph = S.foldl (\acc checkParti -> case checkParti of
                                                                    False -> False
                                                                    True -> acc) True checkPartitions
                            where checkPartitions = S.map (\parti -> (isModule parti graph)) partition
...
-- se aleg doar partitile modulare
-- se elemina partitia de lungime 1
maximalModularPartition partitions graph = let
                                            allModular = filter (\part -> isModularPartition part graph) partitions
                                            allWithoutOne = filter (\part -> (length part) > 1) allModular
                                        in minimumBy (compare `on` length) allWithoutOne
